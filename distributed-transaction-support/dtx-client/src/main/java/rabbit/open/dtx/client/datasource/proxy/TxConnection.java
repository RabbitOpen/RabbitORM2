...
public class TxConnection implements Connection {
    ...
    public PreparedStatement prepareStatement(String sql) throws SQLException {
        SQLMeta sqlMeta = SimpleSQLParser.parse(sql);
        PreparedStatement realStmt;
        boolean autoGenKey = false;
        if (sqlMeta.getSqlType() == SQLType.INSERT && txDataSource.isAutoIncrement(sqlMeta.getTargetTables(), realConn)) {
            realStmt = realConn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);
            autoGenKey = true;
        } else {
            realStmt = realConn.prepareStatement(sql);
        }
        TxPreparedStatement statement = getTxPreparedStatement(realStmt, sql);
        statement.setAutoGenKey(autoGenKey);
        return statement;
    }
    ...
    public PreparedStatement prepareStatement(String sql, int autoGeneratedKeys) throws SQLException {
        ...
        TxPreparedStatement txPreparedStatement = getTxPreparedStatement(statement, sql);
        txPreparedStatement.setAutoGenKey(Statement.RETURN_GENERATED_KEYS == autoGeneratedKeys);
        return txPreparedStatement;
    }
