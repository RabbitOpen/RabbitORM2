...
public class AgentDelegator {
    ...
    public static Object intercept(@Origin Method method, @SuperCall Callable<?> callable,
                                   @AllArguments Object[] args, @This Object obj) throws Throwable {
        if (obj instanceof EnhanceRemission) {
            return callable.call();
        }
        Map<DelegationHandler, Object> result = new HashMap<>();
        handlers.forEach(h -> {
            result.put(h, h.before(method, args));
        });
        try {
            // 原有函数执行
            Object ret = callable.call();
            handlers.forEach(h -> {
                h.after(result.get(h), method, args);
            });
            return ret;
        } catch (Throwable t) {
            handlers.forEach(h -> {
                h.error(result.get(h), t, method, args);
            });
            throw t;
        }
    }
