...
public class SQLiteTransactionTest {
    ...
    public void transactionAssert() {
        if (counter.containsKey("springTransactionTest")) {
            TestCase.assertEquals(counter.get("springTransactionTest") + 2, us
                    .createQuery().count());
        }
        if (counter.containsKey("annotationRollbackTest2")) {
            TestCase.assertEquals(counter.get("annotationRollbackTest2")
                    .intValue(), us.createQuery().count());
        }
     ...
     */
    @Test
    @Transactional(readOnly = true, transactionManager="transactionManager-sqlite")
    @Rollback(true)
