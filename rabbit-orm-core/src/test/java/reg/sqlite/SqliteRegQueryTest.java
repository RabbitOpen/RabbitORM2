...
@ContextConfiguration(locations = { "classpath:applicationContext-sqlite-reg.xml" })
public class SqliteRegQueryTest {

	@Autowired
	SqliteRegUserService rus;
	
	@Autowired
	SqliteRegRoomService rrs;
	...
	public void regQueryTest() {
		...
		TestCase.assertEquals(1, rus.createQuery()
				...
				// 起始日期和截止日期相差2天
				.addFilter("JULIANDAY(${end}) - JULIANDAY(${start})", 2)
				.count());
		...
		RegUser unique = rus.createQuery().addFilter("${to} - ${from}", user.getTo() - user.getFrom(), FilterType.LTE)
			...
			.addJoinFilter("JULIANDAY(${end}) - JULIANDAY(${start})", 2, RegRoom.class)
