...
public class Session extends AbstractConnection{
	...
	public void destroy(){
		...
		logger.info("session{stmtments:" + cachedStmts.size() + 
				"} closed, [" + dataSource.getCounter() + "] session left!");
	}
	...
	@Override
	public PreparedStatement prepareStatement(String sql, int genKey) throws SQLException{
		...
		if(cachedStmts.size() == dataSource.getMaxCachedStmt()){
			PreparedStatement idleStmt = cachedStmts.remove(cachedStmts.keySet().iterator().next());
			idleStmt.close();
		...
		if(DO_NOTHING == genKey){
			...
			stmt = PreparedStatementProxy.getProxy(conn.prepareStatement(sql, genKey));
		}
